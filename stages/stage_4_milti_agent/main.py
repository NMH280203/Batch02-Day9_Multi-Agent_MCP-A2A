...
def check_routing(state: LegalState) -> list[Send]:
    """Conditional routing: dispatch specialist agents based on keywords."""
    ...
    print(f"\n  [Node: check_routing] Dispatching {len(tasks) or 1} route(s)")
...
async def privacy_agent(state: LegalState) -> dict:
    ...
    print("\n  [Node: privacy_agent] Privacy specialist analysing GDPR/data issues...")
    llm = get_llm()
